...
export default function CounselorDashboardPage() {
  ...
  const upcomingSessions = appointments.filter(a => {
    const sessionDate = parseISO(a.date);
    const isUpcoming = isToday(sessionDate) || isFuture(sessionDate);
    const isActiveStatus = a.status.toLowerCase() !== 'completed' && a.status.toLowerCase() !== 'cancelled';
          ...
          <Card className="flex flex-col shadow-lg">
            ...
            <CardContent className="flex-1 flex flex-col justify-center items-center text-center">
              ...
              <div className="text-md text-muted-foreground flex items-center gap-2 mt-1">
               ...
               <Badge variant={nextSession.status.toLowerCase() === 'pending' ? 'secondary' : 'default'} className="mt-3">
                {nextSession.status}
              </Badge>
            </CardContent>
            <CardFooter>
              <Button className="w-full" asChild size="lg" disabled={nextSession.status.toLowerCase() === 'pending'}>
